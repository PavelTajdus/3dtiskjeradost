---
title: "Pod tryskou 2/2025: Od automobilového průmyslu po domácí projekty"
pubDate: "2025-02-05T15:33:00.000Z"
updatedDate: "2025-04-20T14:48:33.000Z"
description: "BMW implementuje 3D tištěnou středovou konzoli do vozidel 2027"
tags: ["Newsletter"]
---

## BMW implementuje 3D tištěnou středovou konzoli do vozidel 2027

BMW plánuje v roce 2027 zavést do výroby vozidlo s jednou z největších 3D tištěných polymerových součástí v automobilovém průmyslu – středovou konzolí. Tento díl, oceněný na SPE Central Europe Automotive Award 2024, je vyroben pomocí technologie velkoformátového aditivního tisku (LFAM) a nahrazuje sedm samostatných komponent jediným kusem. Výsledkem je snížení složitosti výroby, úspora materiálu a redukce uhlíkové stopy.![](https://3dtiskjeradost.cz/content/images/2025/04/image-18.png)

Konzole váží přibližně 4 kg, což je o 30 % méně než u tradiční verze, díky použití recyklovaných a obnovitelných materiálů. BMW plánuje ročně vytisknout až 18 000 kusů pomocí robotických tiskáren od Hans Weber Maschinenfabrik. Tato inovace nejen zjednodušuje proces montáže, ale také přispívá k energetické efektivitě vozidel a udržitelnosti výroby.

PS: Na Formnext 2024 jsem viděl stánek Weber (fotky v článku jsou právě z Fomnextu) a opravdu bylo na co se dívat.

*Zdroj*: [https://www.ptonline.com/articles/bmw-group-vehicle-to-adopt-3d-printed-center-console-2](https://www.ptonline.com/articles/bmw-group-vehicle-to-adopt-3d-printed-center-console-2)## Designový lustr z biologicky odbouratelného PLA vytvořený 3D tiskem

Mickus Projects představili nový lustr Hedron Pendant Chandelier vyrobený z bioplastu. Tento designový kousek je nejen esteticky působivý, ale také šetrný k životnímu prostředí díky použití udržitelných materiálů. Bioplast, ze kterého je lustr vyroben, je plně biologicky odbouratelný, což ho činí ideální volbou pro ty, kdo hledají ekologická řešení v interiérovém designu. (poznámka já: Jak krásně jde popsat PLA, že?)![](https://3dtiskjeradost.cz/content/images/2025/04/image-19.png)

Lustr má geometrický tvar inspirovaný polyhedry a působí jako umělecký prvek v jakémkoli prostoru. Jeho výroba ukazuje, jak lze spojit moderní technologie s odpovědným přístupem k přírodním zdrojům. Pokud hledáte způsob, jak dodat svému interiéru jedinečný vzhled a zároveň podpořit udržitelnost, tento lustr by mohl být skvělou volbou.

*Zdroj*: https://www.dezeen.com/2025/01/17/mickus-projects-bioplastic-hedron-pendant-chandelier/## 3D tištěný přenosný notebook s Raspberry Pi 5 a výdrží až 3 hodiny

Máte doma Raspberry Pi 5 a přemýšlíte, jak ho využít na maximum? Tvůrce "el_cuadillo" vytvořil plně funkční přenosný notebook s 3D tištěným pouzdrem, který si můžete snadno vzít kamkoliv s sebou. Tento "laptop" má 10,1" dotykový displej, úložiště pro bezdrátovou klávesnici, myš i herní ovladač a běží na baterii složené ze čtyř článků 18650, což mu zajišťuje výdrž 2–3 hodiny.![](https://3dtiskjeradost.cz/content/images/2025/04/image-20.png)

Notebook je navržen v OnShape a vytištěn na tiskárně Bambu A1 z materiálů Sunlu PLA+ a Giantarm PLA Silk. Uvnitř najdete nejen Raspberry Pi 5 s aktivním chlazením, ale také UPS modul Geekworm, který umožňuje bezpečné vypnutí systému pomocí Python skriptů. A co je nejlepší? Tvůrce plánuje sdílet STL soubory pro tisk, takže si tento projekt můžete postavit sami.

*Zdroj: *[https://www.hackster.io/news/this-fully-3d-printed-laptop-lets-you-take-your-raspberry-pi-5-wherever-you-go-6aae202b40c1](https://www.hackster.io/news/this-fully-3d-printed-laptop-lets-you-take-your-raspberry-pi-5-wherever-you-go-6aae202b40c1)## SPINC: Projekt 3D tištěné automatické nabíječky baterií s retro designem

Pokud hledáte praktické a stylové řešení pro nabíjení baterií, projekt SPINC od Maximiliana Kerna může být přesně to, co potřebujete. Tento otevřený projekt kombinuje retro design stolních hodin s automatickou nabíječkou pro NiMH AA baterie. Stačí vložit baterii do horní části zařízení a SPINC se postará o zbytek – automaticky ji nabije, zkontroluje polaritu a po dokončení ji vysune do výstupního zásobníku. Díky optickému senzoru a mikroservu zvládne zařízení pojmout až sedm baterií najednou.![](https://3dtiskjeradost.cz/content/images/2025/04/image-21.png)

SPINC je navíc plně přizpůsobitelný – na GitHubu najdete všechny potřebné soubory, včetně STL modelů pro 3D tisk, PCB návrhů a kódu pro Raspberry Pi Pico. Můžete si tak vytvořit vlastní barevnou variantu nebo upravit software podle svých potřeb. Tento projekt je ideální nejen pro domácí použití, ale i jako originální dárek pro technologické nadšence. 

*Zdroj*: [https://www.xda-developers.com/elegant-retro-raspberry-pi-pico-clock-doubles-battery-charger/](https://www.xda-developers.com/elegant-retro-raspberry-pi-pico-clock-doubles-battery-charger/)## PaTS-Wheel: Transformovatelné 3D tištěné kolo překonávající 70 % překážky

PaTS-Wheel je nový typ kola pro mobilní roboty, který kombinuje výhody tradičních kol a „whegs“ (kola s nožkami). Toto pasivně transformovatelné kolo dokáže měnit svůj tvar podle terénu – na hladkém povrchu zůstává kulaté, zatímco na nerovném terénu se přizpůsobí a vytvoří háky, které pomáhají překonávat překážky. Transformace je zajištěna pouze kontaktními silami, bez potřeby senzorů nebo motorů.

Kolo je vyrobeno jako jediný díl pomocí 3D tisku, což usnadňuje jeho výrobu i přizpůsobení různým robotickým platformám. V testech PaTS-Wheel překonalo čistě kruhové kolo i wheg – dokázalo zdolat překážky až do výšky 70 % svého průměru, zatímco wheg zvládl 61 % a běžné kolo pouze 25 %. Navíc má podobnou spotřebu energie jako tradiční kola a generuje méně vibrací, což zlepšuje životnost robota i přesnost jeho senzorů. Tento design představuje efektivní řešení pro pohyb robotů v náročném terénu.

 *Zdroj*: [https://www.youtube.com/watch?v=oZbZZlDDEnc](https://www.youtube.com/watch?v=oZbZZlDDEnc)## Plně automatizovaný 3D tisk s Bambu Lab A1 Mini a Printerhive

Včera na streamu jsem ukazoval plně automatizovaný 3D tisk. Vypadá to sice trochu jako dětská hračka, ale nechte se zmást. Je to opravdu výkonná fabrika!

Tohle **plně automatické** kombo je možné díky tiskárně Bambu lab A1 mini, swap-modu a [Printerhive](https://printerhive.com/cs).
---

            
            

                
                

                    ## Sign up for 3D tisk je radost!
                    

Tvůj zdroj zajímavostí a článků o 3D tisku.
                    
        
            
            

                
                
                    Odebírat
                    
        
            
                
                
                
            
            
                .nc-loop-dots-4-24-icon-o{--animation-duration:0.8s}
                .nc-loop-dots-4-24-icon-o *{opacity:.4;transform:scale(.75);animation:nc-loop-dots-4-anim var(--animation-duration) infinite}
                .nc-loop-dots-4-24-icon-o :nth-child(1){transform-origin:4px 12px;animation-delay:-.3s;animation-delay:calc(var(--animation-duration)/-2.666)}
                .nc-loop-dots-4-24-icon-o :nth-child(2){transform-origin:12px 12px;animation-delay:-.15s;animation-delay:calc(var(--animation-duration)/-5.333)}
                .nc-loop-dots-4-24-icon-o :nth-child(3){transform-origin:20px 12px}
                @keyframes nc-loop-dots-4-anim{0%,100%{opacity:.4;transform:scale(.75)}50%{opacity:1;transform:scale(1)}}
            
        
    
                
            

            

                Email sent! Check your inbox to complete your signup.
            

            

        
        
                    

No spam. Unsubscribe anytime.