---
title: "Pokec o 3D tisku: zákaz 3D tiskáren v USA a CAN problémy na Voronech"
pubDate: "2026-01-20T12:00:00.000Z"
updatedDate: "2026-01-20T12:00:00.000Z"
description: "Na prvním members Pokec streamu roku 2026 jsme rozebrali kontroverzní návrh zákona o omezení 3D tiskáren ve Washingtonu, problémy s dostupností CAN desek pro Vorony a chystané 3D skenování."
heroImage: "/content/images/youtube/CXq_QTO0gfQ.jpg"
tags: ["Youtube streamy", "Members only", "Pokec o 3D tisku"]
draft: false
---

První Pokec o 3D tisku v roce 2026 — nový název pro úterní members streamy, protože "3D tiskové úterý" nikomu nic neřekne. Na programu: šílený návrh zákona z USA, mizející CAN desky pro Vorony a plány na 3D skenování.

> Celý stream si můžete pustit [na YouTube](https://www.youtube.com/watch?v=CXq_QTO0gfQ). Jedná se o members-only obsah — [členství od 45 Kč](https://www.youtube.com/channel/UCACAWyuYlpfH2Jn6HLhhGgw/join).

---

## USA: zákon o omezení 3D tiskáren

Ve Washingtonu DC a státě New York navrhli zákon, který by omezil používání 3D tiskáren. Vzniklo to po vraždě pomocí "ghost gun" (neregistrované zbraně) v roce 2024.

## Co zákon navrhuje:
- Výrobci tiskáren musí implementovat **detekci tisku zbraní** (ve firmware nebo sliceru)
- **Digitální podpis** — nepodepsaný soubor ze sliceru tiskárna nevytiskne
- Prokurátor vytvoří **databázi zakázaných 3D modelů** zbraní
- Týká se nejen 3D tiskáren, ale i **CNC a laserů**
- Porušení: **až 5 let vězení** a pokuta 15 000 dolarů

## Proč je to absurdní:
- Jak firmware pozná vázu od hlavně? Nebo propisku od tlumiče?
- Každý model by musel projít "schvalovacím procesem" — představte si to u desítek tisíc tisků denně
- Vztahuje se i na CNC a lasery — zakážete vyřezat zbraň z překlížky?
- Kuchyňským nožem je spácháno mnohonásobně víc násilných činů než 3D tištěnými zbraněmi

Za mě je to návrh od lidí, kteří 3D tisku vůbec nerozumí a vezou se na vlně popularity technologie. Ale sledujte to — mohlo by to vytvořit precedent i pro jiné země.

---

## Voron: CAN desky mizí z trhu

Praktičtější téma — **FYSTEC omezil výrobu SB Combo V2** a další CAN desky pro Vorony. Důvod? Klesající zájem o Voron tiskárny, výrobci se přesouvají k Bambu a Prusa příslušenství.

## Současná situace:
- FYSTEC — "bude za dva měsíce" (moc tomu nevěřím)
- BigTreeTech — EBB desky existují, ale ne všechny s STM čipem a USB
- LDO — Nighthawk, ale zbytečně komplexní a drahý
- MKS (Maker Base) — nějaké alternativy, ale méně dokumentace
- AliExpress — dojíždějí starší zásoby

**Problém s RP2040 čipem**: nesnesou vysoké teploty v komoře Vorona (nad 60 °C). STM čipy jsou lepší, ale těch desek je málo.

Pokud stavíte Vorony, zásobte se CAN deskami, dokud jsou. A zálohujte si konfigurace — až vám deska odejde, náhrada nebude snadná.

---

## H2C předána zákazníkovi

H2C z minulých streamů už je u zákazníka. Tiskárna byla parádní — za ty peníze dostanete, co máte dostat, a možná trochu víc. Na stole je teď prázdno.

---

## Co chystám

Dostal jsem na tři týdny **3D skener** na testování. V nejbližší době bude stream, kde ho vyzkoušíme. Pokud máte tipy, co skenovat, hoďte to na Discord.

---

## Shrnutí

- **Zákon o omezení 3D tiskáren v USA** je absurdní, ale sledujte vývoj
- **CAN desky pro Vorony** mizí z trhu — zásobte se, dokud můžete
- **H2C** předána, spokojenost na obou stranách
- **3D skenování** — připravuji stream s půjčeným skenerem
- Nový formát members streamů: **Pokec o 3D tisku** (lepší pro vyhledávání)
