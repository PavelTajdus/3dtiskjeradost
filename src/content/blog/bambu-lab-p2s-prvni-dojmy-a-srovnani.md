---
title: "Bambu Lab P2S: první dojmy a proč je to game changer"
pubDate: "2025-10-14T12:00:00.000Z"
updatedDate: "2025-10-14T12:00:00.000Z"
description: "Bambu Lab představilo P2S a já jsem neodolal. Za 18 tisíc dostanete zaboxovanou tiskárnu s AMS2, HD kamerou a automatickou kalibrací průtoku. Srovnal jsem to s Prusa Core One — a výsledek je jasný."
heroImage: "https://img.youtube.com/vi/WtUevWYWlQY/maxresdefault.jpg"
tags: ["Youtube streamy", "Bambu Lab", "Members only", "Pokec o 3D tisku"]
draft: false
---

Bambu Lab představilo P2S a já jsem neodolal — objednal jsem ji ještě před streamem. Za 18 tisíc korun dostanete zaboxovanou tiskárnu s AMS2 se sušením, HD kamerou a spoustou vychytávek. Pojďme se podívat, co všechno nabízí.

> Celý stream si můžete pustit [na YouTube](https://www.youtube.com/watch?v=WtUevWYWlQY). Jedná se o members-only obsah — [členství od 45 Kč](https://www.youtube.com/channel/UCACAWyuYlpfH2Jn6HLhhGgw/join).

---

## Proč P2S mění pravidla hry

Řeknu to na rovinu: za 18 000 Kč (750 €) dostanete tiskárnu, která předčí většinu konkurence v daném segmentu. Za cenu sestaveného Prusa Core One (33 000 Kč) si koupíte **dvě P2S s AMS2**. To je prostě těžký argument.

Nechci křivdit Průšovi — Core One je super tiskárna a stavěl jsem ji minulý týden. Ale čistě z pohledu uživatele, který chce tisknout a ne ladit, je P2S výrazně lepší deal.

---

## Co je na P2S nového

Prošel jsem si specifikace a recenzi od CNC Kitchen. Tady jsou hlavní novinky:

### Dotykový displej
Kdo zná displej z X1, ví, že je parádní. P2S má stejně kvalitní rozhraní — přehledné, rychlé, moderní.

### AMS2 se sušením
Dvojkové AMS s aktivním vyhříváním. Já AMS používám hlavně jako zásobník — dojde filament, natáhne se z dalšího. Funguje parádně a sušení je bonus navíc.

### Vylepšený hotend
High-flow hotend s delší nahřívací zónou než u A1. Nové výměnné trysky jsou zpětně kompatibilní s A1 (ale ne naopak — z A1 nedávat do P2S kvůli jiným výřezům na chlazení).

### Automatická kalibrace průtoku
Speciální servomotor na extruderu dokáže měřit 20 000× za vteřinu. Umí automaticky kalibrovat průtok filamentu, detekovat ucpání hotendu i prokousaný filament.

### Full HD kamera
Konečně pořádná kamera — ne jeden snímek za 5 vteřin jako dřív. S lepší kamerou přišla i AI detekce problémů (špagetifikace, blob). Citlivost jde nastavit.

### Chytrá ventilace
Při tisku ABS se uzavře přívod venkovního vzduchu a teplý vzduch cirkuluje přes filtr. Při PLA se naopak tahá studený vzduch zvenku. Komora se bez aktivního vyhřevu dostane na 55 °C.

### Detaily, které potěší
- **Hladká vnitřní plocha** — žádné hrany, kde se chytá bordel. Prostě setřete a je čisto
- **Držáky** na přenášení, do kterých je integrovaná ventilace
- **Bezpečnostní fólie** na předním skle
- **Vylepšený filament cutter** — vysunuje se, takže můžete tisknout přes celou plochu

---

## Core One vs. P2S: upřímné srovnání

| | Prusa Core One (kit) | Bambu Lab P2S |
|---|---|---|
| **Cena** | 26 900 Kč | ~18 000 Kč |
| **Stav** | Stavebnice (10–16h) | Hotová tiskárna |
| **AMS** | Nemá | AMS2 se sušením |
| **Kamera** | Příslušenství | Full HD v ceně |
| **Auto kalibrace** | Základní | Průtok + mesh + úvodní kalibrace |

Za cenu jednoho Core One máte skoro dvě P2S. Pro farmu, zakázkový tisk nebo prostě když chcete tisknout a ne skládat — je volba jasná.

---

## Pláty z Alíku za pětikilo

Na streamu jsem taky ukázal náhradní pláty pro Core One z AliExpressu. Dva kusy za necelých 500 Kč — oproti originálním prušáckým plátům je to výrazný rozdíl. Povrch je příjemné jemné PEI, ne příliš hrubé, ne saténové. Rozměrově sedí, kvalita je v pohodě.

Originální plát od Průši má na jedné straně povrch podobný Engineering Plate od Bambu, který mi nesedí — zůstávají na něm otisky a stopy i po PLA. Na běžné materiály mi víc vyhovuje klasické PEI.

---

## Shrnutí

- **P2S je nejlepší poměr cena/výkon**, který jsem zatím viděl
- Za 18 000 Kč dostanete zaboxovanou tiskárnu s AMS2, HD kamerou a automatickou kalibrací
- Core One je dobrá tiskárna, ale cenově se v tuhle chvíli nevyplatí
- Náhradní pláty z Alíku za 500 Kč fungují skvěle
- P2S už je na cestě ke mně — těšte se na podrobný stream
